...
export function normalizeFeatureIds(features) {
    ...
    if (out.has('unlimited_bookings'))
        out.delete('bookings_monthly_5');
    return [...out];
}
export function applyPlanFlagsToLawyer(lawyer, planFeatures) {
...
export function monthlyBookingLimit(planFeatures) {
    const ids = normalizeFeatureIds(planFeatures);
    if (ids.includes('unlimited_bookings'))
        return null;
    if (ids.includes('bookings_monthly_5'))
        return 5;
    return 0;
}
