...
publicRouter.get('/qa/:slug/answers', async (req, res) => {
    try {
        const postRes = await query('SELECT id, slug, title, excerpt, category, content FROM qa_posts WHERE slug = $1 AND status = $2', [req.params.slug, 'published']);
        const post = postRes.rows[0];
        if (!post) {
            res.status(404).json({ detail: 'Question not found' });
            return;
        }
        const answersRes = await query(`SELECT id, lawyer_id, lawyer_name, body, created_at
       FROM qa_answers WHERE qa_post_id = $1 AND status = 'published'
       ORDER BY created_at ASC`, [post.id]);
        res.json({
            question: {
                id: post.id,
                slug: post.slug,
