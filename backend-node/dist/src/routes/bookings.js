...
bookingsRouter.post('/', async (req, res) => {
    try {
        ...
        if (limit !== null) {
            ...
            const count = cms.bookings.filter((b) => b.lawyerId === lawyer.id &&
        ...
        const sessionUser = verifyUserSessionToken(req.cookies?.lawyerspot_user_session);
        const lawyerName = sanitizeText(body.lawyerName ?? lawyer.name, 120);
        const booking = {
            id: `b-${Date.now()}`,
