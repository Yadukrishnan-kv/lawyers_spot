...
function findColumn(mapped, ...candidates) {
    for (const c of candidates) {
        if (mapped[c] && mapped[c].length > 0)
            return mapped[c];
    }
    return null;
}
...
pendingLawyersRouter.get('/stats', async (_req, res) => {
    try {
        const r = await query(`SELECT status, count(*)::int as count FROM lawyer_pending GROUP BY status`);
...
pendingLawyersRouter.get('/districts', async (_req, res) => {
    try {
        const r = await query(`SELECT DISTINCT district FROM lawyer_pending WHERE district IS NOT NULL AND district != '' ORDER BY district`);
...
pendingLawyersRouter.post('/import', upload.single('file'), async (req, res) => {
    try {
        ...
        if (rows.length === 0)
            return res.status(400).json({ detail: 'No data rows found' });
        ...
        for (const r of mobileRes.rows)
            existingMobiles.add(r.mobile.toLowerCase());
        ...
        for (const r of enrollRes.rows)
            existingEnrollments.add(r.enrollment_no.toLowerCase());
        for (let i = 0; i < rows.length; i++) {
            ...
            if (mapped.mobile)
                existingMobiles.add(mapped.mobile.toLowerCase());
...
pendingLawyersRouter.post('/:id/verify', async (req, res) => {
    try {
        ...
        const practiceList = pending.practice_areas
            ? pending.practice_areas.split(',').map((s) => s.trim()).filter(Boolean)
            : [];
        ...
        try {
          ...
          profile_faq, subscription_plan_id, subscription_expires_at, featured, top_rated
            ...
            await client.query(`INSERT INTO platform_users (id, email, password_hash, name, role, lawyer_id, phone, status)
         VALUES ($1, $2, $3, $4, 'lawyer', $5, $6, 'active')`, [userId, pending.email || `${lawyerId}@lawyerspot.com`, passwordHash, pending.name, lawyerId, pending.mobile ?? null]);
            await client.query(`UPDATE lawyer_pending SET status = 'verified', verified_by = $1, verified_at = NOW(), updated_at = NOW() WHERE id = $2`, [req.cookies?.lawyerspot_admin_session ? 'admin' : 'admin', req.params.id]);
...
pendingLawyersRouter.post('/:id/reject', async (req, res) => {
    try {
        ...
        if (r.rows[0].status === 'rejected')
            return res.status(400).json({ detail: 'Already rejected' });
        await query(`UPDATE lawyer_pending SET status = 'rejected', rejection_reason = $1, rejected_by = $2, rejected_at = NOW(), updated_at = NOW() WHERE id = $3`, [sanitizeText(reason, 1000), 'admin', req.params.id]);
