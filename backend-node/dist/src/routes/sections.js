...
sectionsRouter.get('/sections', async (_req, res) => {
    try {
        ...
        const result = await query(`SELECT id, type, section_number, title, slug, body, punishment,
...
sectionsRouter.get('/sections/:slug', async (req, res) => {
    try {
        const result = await query(`SELECT id, type, section_number, title, slug, body, punishment,
              category, status, display_order, created_at
       FROM sections
       WHERE slug = $1 AND status = 'active'`, [req.params.slug]);
...
sectionsRouter.get('/admin/sections/:id(\\d+)', requireAdmin, async (req, res) => {
    try {
        const result = await query(`SELECT id, type, section_number, title, slug, body, punishment,
              category, status, display_order, created_at, updated_at
       FROM sections WHERE id = $1`, [Number(req.params.id)]);
        if (result.rows.length === 0) {
            res.status(404).json({ detail: 'Section not found' });
            return;
        }
...
sectionsRouter.post('/admin/sections', requireAdmin, async (req, res) => {
    try {
        const { type, section_number, title, slug, body, punishment, category, status, display_order } = req.body;
        if (!type || !['ipc', 'bns'].includes(type)) {
        ...
        const result = await query(`INSERT INTO sections (type, section_number, title, slug, body, punishment, category, status, display_order)
...
sectionsRouter.put('/admin/sections/:id(\\d+)', requireAdmin, async (req, res) => {
    try {
        const { section_number, title, slug, body, punishment, category, status, display_order } = req.body;
        const existing = await query('SELECT slug FROM sections WHERE id = $1', [Number(req.params.id)]);
        if (existing.rows.length === 0) {
            res.status(404).json({ detail: 'Section not found' });
            return;
        }
        const result = await query(`UPDATE sections SET
...
sectionsRouter.delete('/admin/sections/:id(\\d+)', requireAdmin, async (req, res) => {
    try {
        const result = await query(`UPDATE sections SET status = 'inactive', updated_at = NOW()
       WHERE id = $1 AND status = 'active'
       RETURNING id, type, section_number, title, slug`, [Number(req.params.id)]);
        if (result.rows.length === 0) {
