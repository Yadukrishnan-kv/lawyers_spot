...
async function findClientByEmail(email) {
    const r = await query(`SELECT id AS user_id, email, phone FROM platform_users
...
async function findClientByPhone(digits) {
    const r = await query(`SELECT id AS user_id, email, phone, count(*) OVER () AS match_count
       FROM platform_users
...
async function resendCooldownRemaining(userId) {
    const existing = await query('SELECT otp_sent_at FROM client_password_resets WHERE user_id = $1', [userId]);
...
clientPasswordResetRouter.post('/verify-otp', verifyOtpLimiter, async (req, res) => {
    try {
        ...
        if (channel === 'email') {
            ...
            const r = await query(`SELECT id, user_id, otp_hash, otp_attempts FROM client_password_resets
          WHERE channel = 'email' AND lower(email) = $1 AND expires_at > NOW()`, [normalized]);
            session = r.rows[0];
        }
            ...
            const r = await query(`SELECT id, user_id, otp_hash, otp_attempts FROM client_password_resets
          WHERE channel = 'phone' AND phone_key = $1 AND expires_at > NOW()`, [digits]);
            session = r.rows[0];
            if (session) {
        ...
        if (channel === 'email') {
            ...
            if (!session.otp_hash || sha256(cleanCode) !== session.otp_hash) {
                await query('UPDATE client_password_resets SET otp_attempts = otp_attempts + 1 WHERE id = $1', [session.id]);
        ...
        const expiresAt = new Date(Date.now() + config.passwordResetTtlMinutes * 60 * 1000);
        await query(`UPDATE client_password_resets
         SET otp_verified = true, otp_hash = NULL, token_hash = $1, token_used = false, expires_at = $2
       WHERE id = $3`, [sha256(token), expiresAt, session.id]);
...
clientPasswordResetRouter.post('/set-password', setPasswordLimiter, async (req, res) => {
    try {
        ...
        const r = await query(`SELECT id, user_id FROM client_password_resets
        WHERE token_hash = $1 AND otp_verified = true AND token_used = false
          AND expires_at > NOW()`, [sha256(resetToken)]);
        const row = r.rows[0];
        if (!row) {
        ...
        const updated = await query(`UPDATE platform_users SET password_hash = $1 WHERE id = $2 AND role = 'client'`, [passwordHash, row.user_id]);
        ...
        await query('UPDATE client_password_resets SET token_used = true, token_hash = NULL WHERE id = $1', [row.id]);
