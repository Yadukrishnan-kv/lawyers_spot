...
import { clearUserCookie, findUserByEmail, findUserByPhone, findUserById, hashPassword, setUserCookie, verifyPassword, requireUser, } from '../platform-auth.js';
import { query } from '../db.js';
import { config } from '../config.js';
import { normalizeEmail, normalizePhone, phoneDigits, sanitizeText, validatePassword, } from '../security/validate.js';
...
async function findPendingSignup(role, pendingId) {
    const r = await query(`SELECT id, role, name, email, password_hash, phone, phone_key, bar_id, city_slug, practice,
            phone_verified, email_otp_hash, email_otp_attempts
       FROM signup_verifications
      WHERE id = $1 AND role = $2 AND expires_at > NOW()`, [pendingId, role]);
    return r.rows[0] ?? null;
}
async function resendCooldownRemaining(column, pendingId) {
    const r = await query(`SELECT ${column} AS sent_at FROM signup_verifications WHERE id = $1`, [pendingId]);
...
authRouter.post('/signup', sensitiveLimiter, async (req, res) => {
    try {
        ...
        const id = randomUUID();
        await query(`INSERT INTO signup_verifications
       ...
       ON CONFLICT (role, lower(email)) DO UPDATE SET
         ...
         expires_at = EXCLUDED.expires_at`, [id, cleanName, normalized, await hashPassword(pwd), e164, digits, expiresAt]);
        const row = await query(`SELECT id FROM signup_verifications WHERE role = 'client' AND lower(email) = $1`, [normalized]);
...
authRouter.post('/signup/verify-phone', otpLimiter, async (req, res) => {
    try {
        ...
        await query(`INSERT INTO platform_users (id, email, password_hash, name, role, phone, status, phone_verified)
       VALUES ($1,$2,$3,$4,'client',$5,'active', true)`, [id, pending.email, pending.password_hash, pending.name, pending.phone]);
...
authRouter.post('/lawyer-signup', sensitiveLimiter, async (req, res) => {
    try {
        const { name, email, password, phone, practice, barId, citySlug } = req.body;
        const normalized = email ? normalizeEmail(email) : null;
        const pwd = password ? validatePassword(password) : null;
        const cleanName = name ? sanitizeText(name, 120) : '';
       ...
       ON CONFLICT (role, lower(email)) DO UPDATE SET
         ...
         phone_otp_sent_at = NOW(),
         expires_at = EXCLUDED.expires_at`, [id, cleanName, normalized, await hashPassword(pwd), e164, digits, cleanBarId, safeCity, cleanPractice, expiresAt]);
        const row = await query(`SELECT id FROM signup_verifications WHERE role = 'lawyer' AND lower(email) = $1`, [normalized]);
        res.json({ success: true, pendingId: row.rows[0].id });
    }
    catch (e) {
        console.error(e);
...
authRouter.post('/lawyer-signup/verify-email', otpLimiter, async (req, res) => {
    try {
        ...
        const safeCity = pending.city_slug && cms.cities.some((c) => c.slug === pending.city_slug)
            ? pending.city_slug
            : cms.cities[0]?.slug;
        ...
        await query(`INSERT INTO platform_users (id, email, password_hash, name, role, lawyer_id, phone, status, phone_verified)
       VALUES ($1,$2,$3,$4,'lawyer',$5,$6,'active', true)`, [userId, pending.email, pending.password_hash, pending.name, lawyerId, pending.phone]);
        await query('DELETE FROM signup_verifications WHERE id = $1', [pending.id]);
...
authRouter.post('/login', sensitiveLimiter, async (req, res) => {
    try {
        ...
        if (rawIdentifier.includes('@')) {
            ...
            if (digits)
                user = await findUserByPhone(digits);
        }
