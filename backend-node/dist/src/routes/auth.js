...
import { clearUserCookie, findUserByEmail, findUserById, hashPassword, setUserCookie, verifyPassword, requireUser, } from '../platform-auth.js';
...
authRouter.post('/signup', sensitiveLimiter, async (req, res) => {
    try {
        ...
        await query(`INSERT INTO platform_users (id, email, password_hash, name, role, status) VALUES ($1,$2,$3,$4,'client','active')`, [id, normalized, await hashPassword(pwd), cleanName]);
...
authRouter.post('/lawyer-signup', sensitiveLimiter, async (req, res) => {
    try {
        ...
        const safeCity = citySlug && cms.cities.some((c) => c.slug === citySlug) ? citySlug : cms.cities[0]?.slug;
        ...
        await query(`INSERT INTO platform_users (id, email, password_hash, name, role, lawyer_id, phone, status) VALUES ($1,$2,$3,$4,'lawyer',$5,$6,'active')`, [userId, normalized, await hashPassword(pwd), cleanName, lawyerId, phone ? sanitizeText(phone, 20) : null]);
