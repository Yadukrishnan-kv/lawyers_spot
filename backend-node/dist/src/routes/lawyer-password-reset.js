 ...
 */
async function findVerifiedLawyerByPhone(digits) {
    const r = await query(`SELECT u.id AS user_id, u.status AS user_status, l.id AS lawyer_id,
            count(*) OVER () AS match_count
       FROM lawyers l
       JOIN platform_users u ON u.lawyer_id = l.id AND u.role = 'lawyer'
      WHERE right(regexp_replace(l.phone, '\\D', '', 'g'), 10) = $1`, [digits]);
    if (r.rows.length !== 1)
        return null; // not found or ambiguous
    const row = r.rows[0];
    if (Number(row.match_count) !== 1)
        return null; // ambiguous phone
    if (row.user_status !== 'active')
        return null; // blocked / deleted
    return row;
}
...
lawyerPasswordResetRouter.post('/request-otp', requestOtpLimiter, async (req, res) => {
    try {
        ...
        const existing = await query('SELECT otp_sent_at FROM lawyer_password_resets WHERE phone_key = $1', [digits]);
        ...
        await query(`INSERT INTO lawyer_password_resets
       ...
       ON CONFLICT (phone_key) DO UPDATE SET
         ...
         token_hash = NULL,
         token_used = false,
         expires_at = EXCLUDED.expires_at`, [randomUUID(), lawyer.user_id, lawyer.lawyer_id, e164, digits, expiresAt]);
        res.json(NEUTRAL_OTP_RESPONSE);
    }
    catch (e) {
...
lawyerPasswordResetRouter.post('/verify-otp', verifyOtpLimiter, async (req, res) => {
    try {
        ...
        const existing = await query('SELECT id, expires_at FROM lawyer_password_resets WHERE phone_key = $1', [digits]);
        ...
        const expiresAt = new Date(Date.now() + config.passwordResetTtlMinutes * 60 * 1000);
        await query(`UPDATE lawyer_password_resets
         SET otp_verified = true, token_hash = $1, token_used = false, expires_at = $2
       WHERE id = $3`, [tokenHash(token), expiresAt, row.id]);
...
lawyerPasswordResetRouter.post('/set-password', setPasswordLimiter, async (req, res) => {
    try {
        ...
        const r = await query(`SELECT id, user_id FROM lawyer_password_resets
        WHERE token_hash = $1 AND otp_verified = true AND token_used = false
          AND expires_at > NOW()`, [tokenHash(resetToken)]);
        const row = r.rows[0];
        if (!row) {
        ...
        const updated = await query(`UPDATE platform_users SET password_hash = $1 WHERE id = $2 AND role = 'lawyer'`, [passwordHash, row.user_id]);
        ...
        await query('UPDATE lawyer_password_resets SET token_used = true, token_hash = NULL WHERE id = $1', [row.id]);
