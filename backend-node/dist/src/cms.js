...
import { DEFAULT_SUBSCRIPTION_PLANS, planRowToJson, } from './subscription-plans.js';
import { applyPlanFlagsToLawyer } from './subscription-features.js';
function lawyerRowToJson(row) {
    return {
        id: row.id,
        slug: row.slug,
        name: row.name,
        ...
        subscriptionPlanId: row.subscription_plan_id ?? 'basic',
...
async function saveSubscriptionPlans(client, plans) {
    for (const p of plans) {
        await client.query(`INSERT INTO subscription_plans (
...
export async function loadCms() {
    ...
    const [stats, practiceAreas, states, cities, lawyers, qaPosts, articles, articleLawyers, topics, reviews, adminUsers, bookings, plansRes,] = await Promise.all([
        ...
        query('SELECT author, rating, text, date, verified, avatar FROM default_profile_reviews ORDER BY sort_order'),
        query('SELECT id, name, email, role, last_login FROM admin_users'),
        query('SELECT id, user_id, lawyer_id, lawyer_name, client_name, client_email, date, time, type, status FROM bookings'),
        query('SELECT id, name, price_monthly, currency, description, features, highlight, sort_order, active FROM subscription_plans ORDER BY sort_order'),
    ]);
    const assignedByArticle = new Map();
    for (const row of articleLawyers.rows) {
        const slug = row.article_slug;
        const lid = row.lawyer_id;
        if (!assignedByArticle.has(slug))
            assignedByArticle.set(slug, []);
        assignedByArticle.get(slug).push(lid);
    }
    const subscriptionPlans = plansRes.rows.length > 0
        ? plansRes.rows.map((r) => planRowToJson(r))
    ...
    return {
        ...
        siteContent: site.site_content && Object.keys(site.site_content).length > 0
            ? site.site_content
            : defaultSiteContent,
        stats: stats.rows,
        ...
        subscriptionPlans,
        lawyers: applySubscriptionEntitlementsToLawyers(normalizeLawyerSlugs(lawyers.rows.map((l) => lawyerRowToJson(l))), subscriptionPlans),
        ...
        articles: articles.rows.map((a) => ({
            ...
            lawyerId: a.lawyer_id ?? undefined,
            assignedLawyerIds: assignedByArticle.get(a.slug) ?? [],
        })),
        trendingTopics: topics.rows.map((t) => t.topic),
...
async function clearContentTables(client) {
    const tables = [
        'article_lawyers',
        'articles',
        'bookings',
...
function applySubscriptionEntitlementsToLawyers(lawyers, plans) {
    ...
    return lawyers.map((raw) => {
        ...
        return applyPlanFlagsToLawyer(lawyer, plan?.features);
    });
}
export async function saveCms(payload) {
    const plans = payload.subscriptionPlans?.length ? payload.subscriptionPlans : DEFAULT_SUBSCRIPTION_PLANS;
    const lawyers = applySubscriptionEntitlementsToLawyers(normalizeLawyerSlugs(payload.lawyers), plans);
    const data = { ...payload, subscriptionPlans: plans, lawyers };
    const incomingIdsSet = new Set(data.lawyers.map((l) => l.id));
    if (data.articles) {
        data.articles = data.articles.map((article) => {
            const art = article;
            const assignedLawyerIds = (art.assignedLawyerIds ?? []).filter((id) => incomingIdsSet.has(id));
    ...
    try {
        ...
        await client.query(`INSERT INTO site_config (id, name, tagline, url, description, site_content, updated_at)
       ...
       ON CONFLICT (id) DO UPDATE SET
        ...
        for (const p of data.practiceAreas) {
            await client.query('INSERT INTO practice_areas (slug, name, icon, lawyers) VALUES ($1, $2, $3, $4)', [p.slug, p.name, p.icon, p.lawyers]);
        }
        for (const s of data.states) {
            await client.query('INSERT INTO states (slug, name, code, active) VALUES ($1, $2, $3, $4)', [s.slug, s.name, s.code, s.active]);
        }
        ...
        await saveSubscriptionPlans(client, data.subscriptionPlans?.length ? data.subscriptionPlans : DEFAULT_SUBSCRIPTION_PLANS);
    ...
    `);
        ...
        if (incomingIds.length > 0) {
            if (existingTables.has('saved_lawyers')) {
                await client.query(`DELETE FROM saved_lawyers WHERE lawyer_id NOT IN (${incomingIds.map((_, i) => `$${i + 1}`).join(',')})`, incomingIds);
            }
            ...
            if (existingTables.has('conversations')) {
                await client.query(`DELETE FROM conversations WHERE lawyer_id NOT IN (${incomingIds.map((_, i) => `$${i + 1}`).join(',')})`, incomingIds);
            }
            await client.query(`DELETE FROM platform_users WHERE lawyer_id IS NOT NULL AND lawyer_id NOT IN (${incomingIds.map((_, i) => `$${i + 1}`).join(',')})`, incomingIds);
            await client.query(`DELETE FROM lawyers WHERE id NOT IN (${incomingIds.map((_, i) => `$${i + 1}`).join(',')})`, incomingIds);
        }
        else {
            if (existingTables.has('saved_lawyers')) {
        ...
        for (const l of data.lawyers) {
            const row = l;
            await client.query(`INSERT INTO lawyers (
        ...
        ON CONFLICT (id) DO UPDATE SET
          ...
          top_rated = EXCLUDED.top_rated`, [
                ...
                row.subscriptionExpiresAt ? new Date(row.subscriptionExpiresAt) : null,
        ...
        for (const a of data.articles) {
            await client.query(`INSERT INTO articles (slug, title, excerpt, category, author, date, read_time, image, trending, status, content, lawyer_id)
         ...
         ON CONFLICT (slug) DO UPDATE SET
           ...
           lawyer_id = EXCLUDED.lawyer_id`, [
            ...
            const assigned = a.assignedLawyerIds ?? [];
            if (assigned.length > 0) {
                for (const lawyerId of assigned) {
                    await client.query(`INSERT INTO article_lawyers (article_slug, lawyer_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [a.slug, lawyerId]);
                }
            }
        }
        ...
        for (let i = 0; i < data.defaultProfileReviews.length; i++) {
            ...
            await client.query(`INSERT INTO default_profile_reviews (author, rating, text, date, verified, avatar, sort_order)
         VALUES ($1,$2,$3,$4,$5,$6,$7)`, [r.author, r.rating, r.text, r.date, r.verified ?? false, r.avatar ?? null, i]);
        }
        for (const u of data.adminUsers) {
            await client.query('INSERT INTO admin_users (id, name, email, role, last_login) VALUES ($1,$2,$3,$4,$5)', [u.id, u.name, u.email, u.role, u.lastLogin ?? null]);
        }
        for (const b of data.bookings) {
            await client.query(`INSERT INTO bookings (id, user_id, lawyer_id, lawyer_name, client_name, client_email, date, time, type, status)
