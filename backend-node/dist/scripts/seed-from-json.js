...
async function main() {
    ...
    const raw = JSON.parse(fs.readFileSync(cmsJson, 'utf-8'));
    if (fs.existsSync(lawyersSample)) {
        raw.lawyers = JSON.parse(fs.readFileSync(lawyersSample, 'utf-8'));
    ...
    if (!raw.siteContent) {
        ...
        raw.siteContent = defaultSiteContent;
    }
    console.log('Seeding CMS data from cms.json...');
    await saveCms(raw);
    const result = await loadCms();
    console.log(`Done: ${result.states.length} states, ${result.cities.length} cities, ${result.lawyers.length} lawyers`);
    await pool.end();
}
main().catch((e) => {
    console.error(e);
