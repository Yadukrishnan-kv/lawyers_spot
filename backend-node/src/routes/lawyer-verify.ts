...
async function resolveLawyer(userId: string) {
...
async function cooldownRemaining(column: 'email_otp_sent_at' | 'phone_otp_sent_at', lawyerId: string): Promise<number> {
  const r = await query<{ sent_at: string | null }>(
    `SELECT ${column} AS sent_at FROM lawyer_self_verifications WHERE lawyer_id = $1`,
    [lawyerId],
  );
...
lawyerVerifyRouter.post('/verify-email/request-otp', otpLimiter, requireUser(['lawyer']), async (req, res) => {
  try {
    ...
    const wait = await cooldownRemaining('email_otp_sent_at', lawyer.id as string);
    if (wait > 0) {
    ...
    const sent = await sendEmail(lawyer.email as string, subject, html);
    if (!sent.ok) {
    ...
    await query(
      `INSERT INTO lawyer_self_verifications (lawyer_id, email_otp_hash, email_otp_attempts, email_otp_sent_at, email_expires_at, updated_at)
       ...
       ON CONFLICT (lawyer_id) DO UPDATE SET
         ...
         updated_at = NOW()`,
      [lawyer.id, sha256(code), expiresAt],
    );
...
lawyerVerifyRouter.post('/verify-email/confirm', otpLimiter, requireUser(['lawyer']), async (req, res) => {
  try {
    ...
    const r = await query<{ email_otp_hash: string | null; email_otp_attempts: number; email_expires_at: string | null }>(
      'SELECT email_otp_hash, email_otp_attempts, email_expires_at FROM lawyer_self_verifications WHERE lawyer_id = $1',
      [lawyer.id],
    );
    const row = r.rows[0];
    ...
    if (!row.email_otp_hash || sha256(cleanCode) !== row.email_otp_hash) {
      await query(
        'UPDATE lawyer_self_verifications SET email_otp_attempts = email_otp_attempts + 1 WHERE lawyer_id = $1',
        [lawyer.id],
      );
    ...
    await query(
      'UPDATE lawyer_self_verifications SET email_otp_hash = NULL, email_otp_attempts = 0, email_expires_at = NULL WHERE lawyer_id = $1',
      [lawyer.id],
    );
    res.json({ success: true });
  } catch (e) {
...
lawyerVerifyRouter.post('/verify-phone/request-otp', otpLimiter, requireUser(['lawyer']), async (req, res) => {
  try {
    ...
    const e164 = lawyer.phone ? normalizePhone(lawyer.phone as string, config.otpDefaultCountryCode) : null;
    if (!e164) {
      res.status(400).json({ detail: 'No phone number on file for this account.' });
      return;
    }
    const wait = await cooldownRemaining('phone_otp_sent_at', lawyer.id as string);
    if (wait > 0) {
      res.status(429).json({ detail: `Please wait ${wait}s before requesting another code.` });
      return;
    }
    ...
    await query(
      `INSERT INTO lawyer_self_verifications (lawyer_id, phone_otp_sent_at, updated_at)
       VALUES ($1, NOW(), NOW())
       ON CONFLICT (lawyer_id) DO UPDATE SET phone_otp_sent_at = NOW(), updated_at = NOW()`,
      [lawyer.id],
    );
    res.json({ success: true });
  } catch (e) {
