...
lawyerRouter.get('/dashboard-stats', requireUser(['lawyer']), async (req, res) => {
  try {
    ...
    const lawyerId = account.lawyer.id as string;
    const rating = (account.lawyer.rating as number) ?? 0;

    const [bookingsRes, conversationsRes] = await Promise.all([
      query<{ count: string }>('SELECT COUNT(*)::text as count FROM bookings WHERE lawyer_id = $1', [lawyerId]),
      query<{ count: string }>('SELECT COUNT(*)::text as count FROM conversations WHERE lawyer_id = $1', [lawyerId]),
    ]);
...
lawyerRouter.get('/bookings', requireUser(['lawyer']), async (req, res) => {
  try {
    ...
    const lawyerId = account.lawyer.id as string;
    const r = await query(
      `SELECT id, user_id, client_name, client_email, date, time, type, status 
       FROM bookings 
       WHERE lawyer_id = $1 
    ...
    if (booking.user_id) {
      ...
      await query(
        `INSERT INTO notifications (user_id, title, message, type)
         VALUES ($1, $2, $3, $4)`,
        [booking.user_id, title, msg, status === 'confirmed' ? 'booking_confirmed' : 'booking_cancelled'],
      );
    }
